...
class Database:
    dbConnection = pymysql.connect(host="localhost", user="root", password="", database="tes_treatment_system")
    dbCursor = dbConnection.cursor()

    # def __init__(self):
    # self.dbConnection = pymysql.connect(host="localhost", user="root", password="", database="tes_treatment_system")
    # self.dbCursor = self.dbConnection.cursor()
    # self.dbCursor.execute("CREATE TABLE IF NOT EXISTS patients (id PRIMARYKEY int, firstname text, lastname "
    #                      "text, dateOfBirth text, monthOfBirth text, yearOfBirth text, gender text, "
    #                     "address text, contactNumber text, emailAddress text, bloodType text, Disease text, "
    #                    "doctor text)")
    ...
    def Insert(self, id, firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber,
               emailAddress, bloodType, Disease, doctor):
        self.dbCursor.execute("INSERT INTO patients VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (
            id, firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber, emailAddress,
            bloodType, Disease, doctor))
        self.dbConnection.commit()

    def Update(self, firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber,
               emailAddress, bloodType, Disease, doctor, id):
        self.dbCursor.execute(
            "UPDATE patients SET firstname = %s, lastname = %s, dateOfBirth = %s, monthOfBirth = %s, yearOfBirth = %s, "
            "gender = %s, address = %s, contactNumber = %s, emailAddress = %s, bloodType = %s, Disease = %s, doctor = %s "
            "WHERE id = %s",
            (firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber, emailAddress,
             bloodType, Disease, doctor, id))
        self.dbConnection.commit()

    def Search(self, id):
        self.dbCursor.execute("SELECT * FROM patients WHERE id = %s", (id,))
        searchResults = self.dbCursor.fetchall()
        return searchResults

    def Delete(self, id):
        self.dbCursor.execute("DELETE FROM patients WHERE id = %s", (id,))
        tkinter.messagebox.showinfo("Deleted data", "Successfully Deleted the Patient data in the database")
        self.dbConnection.commit()

    def Display(self):
        self.dbCursor.execute("SELECT * FROM patients")
...
class InsertWindow:
    def __init__(self):
        ...
        self.Disease = tkinter.StringVar()
        ...
        tkinter.Label(self.window, fg=fg_color, bg=bg_color, font=("times new roman", 10, "bold"),
                      text="Disease of Patient", width=25).grid(pady=5, column=1, row=12)
        tkinter.Label(self.window, fg=fg_color, bg=bg_color, font=("times new roman", 10, "bold"),
        ...
        self.DiseaseEntry = tkinter.Entry(self.window, width=25, textvariable=self.Disease)
        ...
        self.DiseaseEntry.grid(pady=5, column=3, row=12)
    ...
    def Insert(self):
        ...
        if (self.test == "SUCCESS"):
            self.database.Insert(self.idEntry.get(), self.firstnameEntry.get(), self.lastnameEntry.get(),
                                 ...
                                 self.DiseaseEntry.get(), self.doctorEntry.get())
    ...
    def Reset(self):
        ...
        self.DiseaseEntry.delete(0, tkinter.END)
...
class UpdateWindow:
    ...
    def Update(self):
        ...
        self.database.Update(self.firstnameEntry.get(), self.lastnameEntry.get(), self.dateOfBirthBox.get(),
                             ...
                             self.emailAddressEntry.get(), self.bloodListBox.get(), self.DiseaseEntry.get(),
...
class DatabaseView:
    def __init__(self, data):
        ...
        self.databaseView["columns"] = (
            ...
            "contactNumber", "emailAddress", "bloodType", "Disease",
            "doctor")
        ...
        self.databaseView.column("Disease", width=100)
...
class HomePage:
    def __init__(self):
        ...
        self.homePageWindow.wm_title("Manage Patients")
        bg_color = "blue"
        fg_color = "white"
        lbl_color = 'GREEN'
