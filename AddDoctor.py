...
class Database:
    ...
    # self.dbCursor.execute("CREATE TABLE IF NOT EXISTS doctors (id PRIMARYKEY int, firstname text, lastname "
    #                      "text, dateOfBirth text, monthOfBirth text, yearOfBirth text, gender text, "
    #                     "address text, contactNumber text, emailAddress text)
    ...
    def Insert(self, id, firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber,
               emailAddress):
        self.dbCursor.execute("INSERT INTO doctors VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (
    ...
    def Update(self, firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber,
        ...
        self.dbCursor.execute(
            "UPDATE doctors SET firstname = %s, lastname = %s, dateOfBirth = %s, monthOfBirth = %s, yearOfBirth = %s, "
            "gender = %s, address = %s, contactNumber = %s, emailAddress = %s, "
            "WHERE id = %s",
    ...
    def Search(self, id):
        self.dbCursor.execute("SELECT * FROM doctors WHERE id = %s", (id,))
        searchResults = self.dbCursor.fetchall()
        return searchResults

    def Delete(self, id):
        self.dbCursor.execute("DELETE FROM doctors WHERE id = %s", (id,))
        tkinter.messagebox.showinfo("Deleted data", "Successfully Deleted the Doctor data in the database")
        self.dbConnection.commit()
...
class InsertWindow:
    def __init__(self):
        ...
        tkinter.Label(self.window, fg=fg_color, bg=bg_color, font=("times new roman", 10, "bold"),
                      text="Doctor Gender", width=25).grid(pady=5, column=1, row=7)
        tkinter.Label(self.window, fg=fg_color, bg=bg_color, font=("times new roman", 10, "bold"),
                      text="Doctor Address", width=25).grid(pady=5, column=1, row=8)
        tkinter.Label(self.window, fg=fg_color, bg=bg_color, font=("times new roman", 10, "bold"),
                      text="Doctor Contact Number", width=25).grid(pady=5, column=1, row=9)
        tkinter.Label(self.window, fg=fg_color, bg=bg_color, font=("times new roman", 10, "bold"),
                      text="Doctor Email Address", width=25).grid(pady=5, column=1, row=10)
...
class UpdateWindow:
    ...
    def Update(self):
        ...
        self.database.Update(self.firstnameEntry.get(), self.lastnameEntry.get(), self.dateOfBirthBox.get(),
                             ...
                             self.emailAddressEntry.get(), self.id)
...
class DatabaseView:
    def __init__(self, data):
        ...
        self.databaseView.heading("id", text="Doctor ID")
...
class HomePage:
    def __init__(self):
        ...
        self.homePageWindow.wm_title("Manage Doctors")
        bg_color = "blue"
        fg_color = "white"
        lbl_color = 'GREEN'
