...
def login():
    if user_name.get() == "" or password.get() == "":
        ...
        try:
            con = pymysql.connect(host="localhost", user="root", password="", database="tes_treatment_system")
            cur = con.cursor()
...
def dashboard():
    ...
    cur.execute("select * from admins where username ='" + user_name.get() + "'")
    row = cur.fetchall()
    ...
    for data in row:
        ...
        d = Button(des, text="Manage Doctor", font="times new roman", command=run1).place(x=300, y=200)
        d.pack()

        p = Button(des, text="Manage Doctor", font="times new roman", command=run2).place(x=300, y=250)
        p.pack()
...
def signup():
    ...
    def action():
        if first_name.get() == "" or last_name.get() == "" or age.get() == "" or city.get() == "" or add.get() == "" or user_name.get() == "" or password.get() == "" or very_pass.get() == "":
            ...
            try:
                con = pymysql.connect(host="localhost", user="root", password="", database="tes_treatment_system")
                cur = con.cursor()
                cur.execute("select * from admins where username=%s", user_name.get())
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror("Error", "User Name Already Exits", parent=winsignup)
                else:
                    cur.execute(
                        "insert into admins(first_name,last_name,age,gender,city,address,username,password) values(%s,%s,%s,%s,%s,%s,%s,%s)",
                        (
                            first_name.get(),
                            last_name.get(),
                            age.get(),
    ...
    winsignup = Tk()
    winsignup.title("tES Treatment System")
...
# app title
win.title("tES Treatment System")
